...
class Schema(object):
    ...
    def __init__(self, columns, ordered = False):
        u"""
        ...
        """
        if not columns:
            raise PanSchInvalidSchemaError(u'An instance of the schema class must have a columns list')

        if not (isinstance(columns, typing.Iterable) and all([isinstance(c, Column) for c in columns])):
            raise PanSchInvalidSchemaError(u'The columns field must be a list of Column objects')

        if not isinstance(ordered, bool):
            raise PanSchInvalidSchemaError(u'The ordered field must be a boolean')
    ...
    def validate(self, df):
        ...
        if self.ordered:
            ...
            column_pairs = izip(series, self.columns)
        else:
            column_pairs = []
            for column in self.columns:
                ...
                if column.name not in df:
                    errors.append(ValidationWarning(
                        u'The column {} exists in the schema but not in the data frame'.format(column.name)))
                    return errors
